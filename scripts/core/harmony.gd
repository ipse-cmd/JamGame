...
# Chord-relative bass vocabulary (V2): the FULL diatonic ladder. Lanes select
# a role against the CURRENT BAR'S CHORD, not a fixed pitch — lane d = chord
# root + d SCALE STEPS (the key's scale context decides major/minor color),
# lane 7 = octave root. V1 was the R/3/5/7/O subset; 2/4/6 are the color
# degrees 26% of real jazz bass lives on (corpus-measured).
const BASS_TONE_NAMES := ["R", "2", "3", "4", "5", "6", "7", "O"]
const BASS_TONE_OCTAVE := 7 # the one lane that isn't "walk up the scale"
...
## Chord-relative bass tone (V2 ladder). Tones 0..6 walk the diatonic scale
## upward from the chord's degree (R 2 3 4 5 6 7 — every interval DIATONIC,
## derived from the key; no chord spelling needed). Tone 7 (O) is the chord
## root an octave up, so the invariant chord_tone_midi(c, 7) ==
## chord_tone_midi(c, 0) + 12 always holds. An empty chord slot (degree -1)
## resolves against the tonic.
static func chord_tone_midi(root_midi: int, chord_degree: int, tone: int) -> int:
	...
	return degree_to_midi(root_midi, deg + tone)
